...
# extracted from https://github.com/Horizon2333/imagenet-autoencoder
__location__ = os.path.realpath(os.path.join(os.getcwd(), os.path.dirname(__file__)))


def load_checkpoint(resume_path, model, startswith=None):
    if os.path.isfile(resume_path):
        ...
        state_dict = {
            ...
            if startswith is None or (startswith and k.startswith(startswith))
        }
...
class ResNetAutoEncoder(nn.Module):
    ...
    def __init__(self, arch='resnet50', skip_connection=False, weights=None):
        super(ResNetAutoEncoder, self).__init__()

        self.encoder = ResNetEncoder(arch)
        self.decoder = ResNetDecoder(arch)

        self.skip_connection = skip_connection

        if weights is not None:
            load_checkpoint(
                os.path.join(__location__, f'{weights}-{arch}.pth'),
                self
            )
...
class ResNetEncoder(nn.Module):
    ...
    def __init__(self, arch='resnet50', weights=None):
...
class ResNetDecoder(nn.Module):
    ...
    def __init__(self, arch='arch', weights=None):
        super(ResNetDecoder, self).__init__()

        configs, bottleneck = get_configs(arch)
        # if configs is None:
        #     configs = get_configs('resnet50')
        #
        # if len(configs) != 4:
        #     raise ValueError("Only 4 layers can be configued")
