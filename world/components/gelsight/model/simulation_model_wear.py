...
from world.components.gelsight.model.crack import create_crack
...
class SimulationApproach:
    ...
    def __init__(self, **config):
        ...
        self.wear_mask = np.zeros((480, 640), dtype=np.float32)

        crack = np.zeros((480, 640))
        crack[:, :] = 1.0

        crack_pt = np.array((random.randint(0, 480), random.randint(0, 640)))
    ...
    def generate(self, obj_depth, rgb, return_depth=False):
        # print('-----------> ', np.shape(obj_depth))
        # cv2.imwrite('object_depth.png', obj_depth)

        # blur rgb background
        wear = cv2.circle(np.zeros((480, 640)), (320, 240), random.randint(50, 250), 1.0, -1)
        kernel = gkern2(15, 1)
        wear = cv2.filter2D(wear, -1, kernel)

        self.wear_mask += wear * 0.03
        self.wear_mask[self.wear_mask > 1.0] = 1.0

        # print(self.background.dtype, rgb.dtype, np.max(self.background), np.max(rgb))
        ...
        cracked_elastomer_depth = elastomer_depth - 0.04 * dilated_crack_mask

        textured_elastomer_depth = gaus_noise(cracked_elastomer_depth, self.texture_sigma)

        out = self.ka * self.worn_background
        ...
        if return_depth:
            return out, cracked_elastomer_depth
        return out
