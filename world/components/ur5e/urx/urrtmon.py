...
class URRTMonitor(threading.Thread):
    ...
    def _transform(self, csys, tcp):
        # Perform the transformation manually without using math3d
        # thank you chatgpt
        
        transformed_pose = [0.0] * 6

        for i in range(3):
            for j in range(3):
                transformed_pose[i] += csys[i][j] * tcp[j]

        for i in range(3, 6):
            transformed_pose[i] = tcp[i]
    ...
    def __recv_rt_data(self):
        ...
        with self._dataAccess:
            ...
            if self._csys:
                ...
                # by chatgpt
                with self._csys_lock:
                    if self._csys is not None and self._tcp is not None:
		        # Perform the transformation without using math3d
                        tcp = self._transform(self._csys, self._tcp)
                        self._tcp = tcp
